...
function addSelectedProductToTable(product, quantity) {
    ...
    if (index > -1) {
        if (selectedProducts[index].quantity > 1) {
            // Decrease quantity and update total amount and profit
            selectedProducts[index].quantity--;
            selectedProducts[index].totalAmount -= selectedProducts[index].price;
            selectedProducts[index].totalProfit -= (selectedProducts[index].price - selectedProducts[index].ogprice);

            // Update the quantity cell in the table
            cell4.textContent = selectedProducts[index].quantity;
            cell5.textContent = (selectedProducts[index].totalAmount).toFixed(2);
            cell6.textContent = (selectedProducts[index].totalProfit).toFixed(2);
        } else {
            // Remove the product from the array if quantity is 1
    ...
    // Update the total amount and profit display
    updateTotalAmount();
    };
