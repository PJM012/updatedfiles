...
/*----------------------SIDE-BAR, NAV-BAR, SWITCH-MODE, TIME-----------------------------*/
...
function editInfo(field) {
  ...
  function uploadProfilePic() {
    ...
    if (input.files && input.files[0]) {
      ...
      reader.onload = function(e) {
        ...
        // Save the image data to local storage for persistence
  ...
  // Function to save the profile picture to local storage
  function saveProfilePic() {
    var profilePic = document.getElementById('profilePic');
    localStorage.setItem('profilePic', profilePic.src);
    document.getElementById('savePicBtn').style.display = 'none'; // Hide save button after saving
    alert('Profile picture saved!'); // Confirmation message
  }
  ...
  // Call loadProfilePic on page load
  window.addEventListener('DOMContentLoaded', loadProfilePic); // Ensures the DOM is fully loaded
